...
dataset_train['Name'] = dataset_train['Name'].str.split(',').str[1]
dataset_train['Name'] = dataset_train['Name'].str.split('.').str[0]
dataset_test['Name'] = dataset_test['Name'].str.split(',').str[1]
dataset_test['Name'] = dataset_test['Name'].str.split('.').str[0]

# droping unnecessary data
dataset_test.drop(['Ticket', 'Cabin' ], axis = 1, inplace = True)
dataset_train.drop(['PassengerId', 'Ticket', 'Cabin' ], axis = 1, inplace = True)
...
df1 = pd.DataFrame(dataset_train['Name'])
df2 = pd.DataFrame(dataset_test['Name'])
nomes = [df1, df2]
nomes = pd.concat(nomes)

labelencoder_name = LabelEncoder()
nomes = labelencoder_name.fit_transform(nomes)
nomes = pd.DataFrame(nomes)
df1 = nomes.iloc[:889, :]
df2 = nomes.iloc[889:, :]
dataset_train['Name'] = df1[0].values
dataset_test['Name'] = df2[0].values

labelencoder_sex = LabelEncoder()
dataset_test.iloc[:, 3] = labelencoder_sex.fit_transform(dataset_test.iloc[:, 3])
dataset_train.iloc[:, 3] = labelencoder_sex.fit_transform(dataset_train.iloc[:, 3])

labelencoder_embarked = LabelEncoder()
dataset_test.iloc[:, 8] = labelencoder_embarked.fit_transform(dataset_test.iloc[:, 8])
dataset_train.iloc[:, 8] = labelencoder_embarked.fit_transform(dataset_train.iloc[:, 8])
...
dataset_train[ dataset_train['Age'].isnull()][['Age', 'AgeFill', 'Sex', 'Pclass']].head(10)
dataset_train['FareFill'] = dataset_train['Fare']
dataset_train[ dataset_train['Fare'].isnull()][['Fare', 'AgeFill', 'Sex', 'Pclass']].head(11)
...
only_final_values.to_csv('titanic_submission_final_2.csv', index = False)
